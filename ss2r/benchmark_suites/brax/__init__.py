...
def render(env, policy, steps, rng, camera=None):
    ...
    videos = []
    for i in range(1):
        ep_trajectory = jax.tree_util.tree_map(lambda x: x[:, i], trajectory.pipeline_state)
        ep_trajectory = pytrees_unstack(ep_trajectory)
        video = image.render_array(
            env.sys, ep_trajectory, camera=camera, height=120, width=160
        )
        videos.append(video)
